...
public class AlumnoDAO {
		...
		if (conexionBD != null) {
			...
			try (
				...
				try (ResultSet resultado = prepararSentencia.executeQuery()) {
					if (resultado.next()) {
						int countSS = resultado.getInt(1);
						int countPP = resultado.getInt(2);
						proyectoInscrito = countSS > 0;
						if (proyectoInscrito == false) {
							proyectoInscrito = countPP > 0;
						}
					}
				}
	...
	public boolean existeAlumno(String correo, String telefono, String matricula, int idAlumnoActual) throws SQLException {
		...
		String consulta = "SELECT COUNT(*) AS cantidad FROM Alumno WHERE (correo = ? OR "
	...
	public static List<Alumno> obtenerAlumnoYProyecto(String nombreBusqueda, String tipoProyecto) throws SQLException {
		...
		if (conexionBD != null) {
			String consulta = "SELECT "
					...
					+ "CASE "
					+ "  WHEN pp.nombreProyecto IS NOT NULL THEN 'Practica Profesional' "
	...
	public static int obtenerIdProyectoPorIdAlumno(int idAlumno) throws SQLException {
		int idProyecto = 0;
		String consulta = "SELECT idProyectoPP AS idProyecto "
				+ "FROM inscripcionee "
				+ "WHERE idAlumno = ? AND idProyectoPP IS NOT NULL "
				+ "UNION "
				+ "SELECT idProyectoSS AS idProyecto "
				+ "FROM inscripcionee "
				+ "WHERE idAlumno = ? AND idProyectoSS IS NOT NULL;";
		...
		try (Connection conexionBD = ConexionBD.abrirConexion(); PreparedStatement sentencia = conexionBD.prepareStatement(consulta)) {
			...
			try (ResultSet resultado = sentencia.executeQuery()) {
				if (resultado.next()) {
					idProyecto = resultado.getInt("idProyecto");
				}
			}
		} catch (SQLException ex) {
			logger.log(Level.SEVERE, "Error al obtener el idProyecto del alumno con id: "
					+ idAlumno, ex);
			throw new SQLException("Error al obtener el idProyecto: "
					+ ex.getMessage(), ex);
		}
	...
	public static Alumno obtenerAlumnoPorId(int idAlumno) {
		Alumno alumno = null;
		String sql = "SELECT nombreAlumno, apellidoAlumno, matricula, correo, telefonoAlumno, promedio "
				+ "FROM Alumno "
				+ "WHERE idAlumno = ?";
		try (Connection conn = ConexionBD.abrirConexion(); PreparedStatement ps = conn.prepareStatement(sql)) {
			...
			try (ResultSet rs = ps.executeQuery()) {
				if (rs.next()) {
					...
					alumno.setTelefonoAlumno(rs.getString("telefonoAlumno"));
