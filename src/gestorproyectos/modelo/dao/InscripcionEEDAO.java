...
public class InscripcionEEDAO {
	...
	public static List<EE> obtenerEEPorProfesor(int idProfesor) throws SQLException {
		...
		if (conexionBD != null) {
			try {
				...
				PreparedStatement prepararConsulta = conexionBD.prepareStatement(consulta);
				prepararConsulta.setInt(1, idProfesor);
				...
				throw new SQLException("Error al obtener las EE asignadas al profesor con id: "
						+ idProfesor, ex);
			} finally {
				conexionBD.close();
			}
	...
	public static List<InscripcionEE> obtenerDetallesInscripcionesPorAlumno(int idAlumno) throws SQLException {
		List<InscripcionEE> inscripciones = new ArrayList<>();
		Connection conexionBD = ConexionBD.abrirConexion();
		...
		if (conexionBD != null) {
			String consulta = "SELECT ie.idInscripcionEE, ie.estadoInscripcion, ee.nombreEE, "
					...
					+ "LEFT JOIN proyectopp pp ON ie.idProyectoPP = pp.idProyectoPP "
					+ "LEFT JOIN proyectoss ss ON ie.idProyectoSS = ss.idProyectoSS "
					+ "WHERE ie.idAlumno = ?";
			...
			try (
				...
				throw new SQLException("Error al obtener las inscripciones del alumno con id: "
						+ idAlumno, ex);
			} finally {
				conexionBD.close();
			}
	...
	public static List<InscripcionEE> obtenerAlumnosSSParaAsignarProyectoSS() throws SQLException {
		...
		if (conexionBD != null) {
			...
			try (
					PreparedStatement prepararSentencia = conexionBD.prepareStatement(consulta); ResultSet resultado = prepararSentencia.executeQuery()) {
				while (resultado.next()) {
					InscripcionEE inscripcion = new InscripcionEE();
	...
	public static List<InscripcionEE> obtenerAlumnosPPParaAsignarProyectoPP() throws SQLException {
		...
		if (conexionBD != null) {
			String consulta = "SELECT DISTINCT "
					...
					+ "INNER JOIN priorizacionproyectos p ON i.idInscripcionEE = p.idInscripcionEE "
					+ "WHERE i.estadoInscripcion = 'inscrito' "
					+ "AND e.nombreEE = 'Practica Profesional' "
					+ "AND (p.idProyectoSS IS NOT NULL OR p.idProyectoPP IS NOT NULL)";
	...
	public static InscripcionEE obtenerInscripcionEE(Integer idAlumno, int idEE) throws SQLException {
		...
		try (Connection conexionBD = ConexionBD.abrirConexion(); PreparedStatement sentencia = conexionBD.prepareStatement(consulta)) {
	...
	public void actualizarEstadoInscripcion() throws SQLException {
		...
		if (conexionBD != null) {
			try {
				...
				try (
						PreparedStatement prepararConsulta = conexionBD.prepareStatement(consulta); ResultSet resultado = prepararConsulta.executeQuery()) {
					while (resultado.next()) {
	...
	public static String obtenerProyectoActivoPorAlumno(int idAlumno) throws SQLException {
		String proyectoActivo = null;
		Connection conexionBD = ConexionBD.abrirConexion();

		if (conexionBD != null) {
			String consulta = "SELECT COALESCE(pp.nombreProyecto, ss.nombreProyecto) AS nombreProyecto "
					+ "FROM inscripcionee ie "
					+ "LEFT JOIN proyectopp pp ON ie.idProyectoPP = pp.idProyectoPP "
					+ "LEFT JOIN proyectoss ss ON ie.idProyectoSS = ss.idProyectoSS "
					+ "WHERE ie.estadoInscripcion = 'Inscrito' AND ie.idAlumno = ? LIMIT 1";
			...
			try (
				...
				try (ResultSet resultSet = preparedStatement.executeQuery()) {
					if (resultSet.next()) {
						proyectoActivo = resultSet.getString("nombreProyecto");
					}
				}
			} finally {
				conexionBD.close();
