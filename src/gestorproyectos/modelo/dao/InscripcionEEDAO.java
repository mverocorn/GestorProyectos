	...
	public static List<InscripcionEE> obtenerAlumnosSSParaAsignarProyectoSS() throws SQLException {
		...
		if (conexionBD != null) {
			String consulta = "SELECT "
					...
					+ "WHERE i.estadoInscripcion = 'Activo' AND e.nombreEE = 'Servicio Social'";
			...
			try (
				...
				while (resultado.next()) {
					...
					inscripcion.setNombreAlumno(resultado.getString("nombreAlumno")); // Nombre del alumno
					inscripcion.setPromedio(resultado.getFloat("promedio")); // Promedio del alumno
					inscripcion.setIdEE(resultado.getInt("idEE")); // ID de la EE
					inscripcion.setNombreEE(resultado.getString("nombreEE")); // Nombre de la EE
					inscripcion.setSeccion(resultado.getString("seccion")); // Sección de la EE
	...
	public static List<InscripcionEE> obtenerAlumnosPPParaAsignarProyectoPP() throws SQLException {
		...
		if (conexionBD != null) {
			String consulta = "SELECT "
					...
					+ "INNER JOIN inscripcionee i ON a.idAlumno = i.idAlumno "
					+ "INNER JOIN ee e ON i.idEE = e.idEE "
					+ "WHERE i.estadoInscripcion = 'Activo' AND e.nombreEE = 'Practica Profesional'";
